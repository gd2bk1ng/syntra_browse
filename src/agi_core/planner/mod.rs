...
//       - Folder-based module for future extensibility.
// ================================================================================================

#![allow(dead_code)]

use crate::agi_core::intent::{IntentPlan, Intent};

// ================================================================================================
// Plan Node (Graph Component)
// ================================================================================================

/// A single node in a plan graph.
///
/// Each node represents a high-level step and may point to subsequent steps.
/// This forms a DAG (Directed Acyclic Graph) for structured reasoning.
#[derive(Debug, Clone)]
pub struct PlanNode {
    pub id: usize,
...
/// A graph of plan nodes representing structured reasoning.
///
/// Nodes are connected linearly by default, but the structure supports branching
/// and merging for more advanced planning strategies.
...
// ================================================================================================
// Planner Engine (High-Level Orchestrator)
// ================================================================================================

/// High-level planning engine that wraps narrative and multi-step planning.
///
/// This is the main entry point used by Cortex lobes.
...
impl PlannerEngine {
    ...
    /// Produce a narrative plan.
    pub fn narrative(&self, intent: &Intent, class: &str) -> String {
        narrative_plan(&intent.text, class)
    }

    /// Produce a multi-step plan and embed it into an IntentPlan.
    pub fn multi_step(&self, intent: &Intent, class: &str) -> IntentPlan {
        let (steps, graph) = multi_step_plan(class, &intent.text);
        IntentPlan::from_steps_and_graph(steps, graph)
    }
}
